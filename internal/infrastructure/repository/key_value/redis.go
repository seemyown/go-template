...
func (r *Repository) Get(key string) (string, error) {
	...
	if err != nil {
		log.Error().Err(err).Msgf("Ошибка получения значения по ключу %s", key)
		return "", err
	}
	return val, nil
}
...
func (r *Repository) Set(key string, val interface{}, ttl int64) error {
	...
	if err != nil {
		log.Error().Err(err).Msgf("Ошибка установки значения %s по ключу %s", val, key)
		return err
	}
	return nil
}
...
func (r *Repository) Del(key string) error {
	...
	if err != nil {
		log.Error().Err(err).Msgf("Ошибка удаления ключа %s", key)
		return err
	}
	return nil
}
